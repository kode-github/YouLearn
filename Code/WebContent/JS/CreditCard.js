...
function cardnumberTest()
{
	...
	console.log(cardVISA);

	var cardMASTERCARD = /^(?:5[1-5][0-9]{14})$/;
	console.log(cardMASTERCARD);

	var cardAMERICANEXPRESS = /^(?:3[47][0-9]{13})$/;
	console.log(cardAMERICANEXPRESS);




	if (inputtxt.match(cardVISA)) {
		if(allLetter(name) == false) return false;
		showAlert('Carta di credito Visa, valida! PAGAMENTO EFFETTUATO');

		return true;
	}

	else if (inputtxt.match(cardMASTERCARD)) {
		if(allLetter(name) == false) return false;
		showAlert('Carta di credito MasterCard, valida! PAGAMENTO EFFETTUATO');
		return true;

	} 

	else if (inputtxt.match(cardAMERICANEXPRESS)) {
		if(allLetter(name) == false) return false;
		showAlert('Carta di credito AmericanExpress, valida! PAGAMENTO EFFETTUATO');
		...
		showAlertERROR('Numero di carta non valido, riprova!');
	}}
...
function allLetter(name) {
	...
	if (name.match(letters)) {
		...
		showAlertERROR('Username must have alphabet characters only');
...
function showAlert(string){
	...
	var div = $("#alert").addClass("alert alert-success");
